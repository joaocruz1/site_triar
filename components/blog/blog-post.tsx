  ...
  return (
    <article className="py-12 bg-white">
      <div className="container mx-auto px-4 max-w-4xl">
        <div className="mb-8">
          <Link href="/blog" className="text-teal-600 hover:text-teal-800 mb-4 inline-block">
            ← Voltar para o blog
          </Link>
          ...
          <Link href={`/blog/categoria/${encodeURIComponent(post.category.toLowerCase())}`}>
            <span className="inline-block px-4 py-1 bg-teal-100 text-teal-700 font-medium rounded-full mb-6">
              {post.category}
            </span>
          </Link>
        </div>
        ...
        <div className="relative w-full h-96 mb-10 rounded-xl overflow-hidden">
          <Image src={post.image || "/images/blog/default.jpg"} alt={post.title} fill className="object-cover" />
        </div>

        <div className={styles.blogContent} dangerouslySetInnerHTML={{ __html: contentHtml }} />
